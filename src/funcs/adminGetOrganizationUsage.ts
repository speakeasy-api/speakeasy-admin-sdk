 ...
 * Returns a billing usage summary by SDK language for a particular organization ID.
 */
export function adminGetOrganizationUsage(
  client: SpeakeasyAdminCore,
  request: operations.GetOrganizationUsageRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.OrganizationUsageResponse,
    | errors.ErrorT
    | APIError
    | SDKValidationError
    | UnexpectedClientError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetOrganizationUsageRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/v1/admin/organizations/{organizationID}/usage")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json("2XX", components.OrganizationUsageResponse$inboundSchema),

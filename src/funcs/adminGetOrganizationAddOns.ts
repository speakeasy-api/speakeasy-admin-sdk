 ...
 */
export function adminGetOrganizationAddOns(
  client: SpeakeasyAdminCore,
  request: operations.GetOrganizationAddOnsRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.GetOrganizationAddOnsResponse,
    | errors.ErrorT
    | APIError
    | SDKValidationError
    | UnexpectedClientError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetOrganizationAddOnsRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/v1/admin/organizations/{organizationID}/add_ons")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json("2XX", operations.GetOrganizationAddOnsResponse$inboundSchema),
